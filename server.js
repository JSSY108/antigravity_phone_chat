...
async function selectChat(cdp, chatTitle) {
    ...
    const EXP = `(async () => {
        try {
            ...
            // 1. Open History Panel (Same robust method as getChatHistory)
            ...
            if (!historyBtn) {
                ...
                // Try position strategy (second button next to new chat)
                if (!historyBtn) {
            ...
            for (let i = 0; i < 15; i++) {
                ...
                // Find a search input or known text to anchor the panel
            ...
            // Give it a tiny bit more time to render list items
            ...
            let candidates = Array.from(panel.querySelectorAll('span, p, div'))
                ...
                .map(el => {
                    ...
                    // Penalty for very short matches that might just be tags
                    if (text.length < 5) score -= 10;
                    
                    // Bonus for DOM depth (more specific element)
                    let depth = 0;
                    let p = el;
            ...
            const executeClick = (targetEl) => {
                ...
                // Dispatch raw mouse events to center of element as fallback
                try {
                ...
                } catch(e) { log('MouseEvent fallback failed: ' + e.message); }
            };
            ...
            // Wait to see if panel closes (meaning selection was successful)
            ...
            // Just hit escape to ensure panel is closed if it somehow stayed open after successful switch
